...
public class SaturnLogger {
    public static final Logger logger = LogManager.getFormatterLogger("saturn");
    ...
    private static MutableText getText(int color) {
        MutableText prefix = Text.literal("[")
                .styled(s -> s.withColor(Formatting.DARK_GRAY.getColorValue()));
        MutableText suffix = Text.literal("SATURN")
                .styled(s -> s.withColor(color));
        MutableText text = Text.literal("] ")
                .styled(s -> s.withColor(Formatting.DARK_GRAY.getColorValue()));

        return prefix.append(suffix).append(text);
    }
